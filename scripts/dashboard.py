...
def generate_charts():
    ...
    plt.figure()
    ax = sns.barplot(data=df, x="FirstName", y="Orders", palette="flare", hue="FirstName", legend=False)
    ax.set_title("Employee Performance", fontsize=20, fontweight='bold', pad=20)
    ...
    print("Generating: Delivery Stats...")
    query = "SELECT DeliveredFlag, COUNT(OrderId) as Count FROM FactOrders GROUP BY DeliveredFlag"
    df = pd.read_sql(query, conn)
    df['Status'] = df['DeliveredFlag'].map({1: 'Delivered', 0: 'Not Delivered'})
    
    plt.figure()
    colors = sns.color_palette("pastel")[0:2]
    plt.pie(df['Count'], labels=df['Status'], colors=colors, autopct='%.0f%%', startangle=90)
    plt.title("Delivery Status", fontsize=20, fontweight='bold', pad=20)
    plt.savefig(f"{FIGURES_DIR}/delivery_stats.png")
    plt.close()

    print("Generating: 3D Global View (Heatmap)...")
    query = """
    SELECT c.Country, MONTH(d.FullDate) as Month, COUNT(f.OrderId) as OrderCount
    FROM FactOrders f
    JOIN DimCustomer c ON f.CustomerId = c.CustomerId
    JOIN DimDate d ON f.DateId = d.DateId
    GROUP BY c.Country, MONTH(d.FullDate)
    """
    df = pd.read_sql(query, conn)
    pivot_table = df.pivot(index="Country", columns="Month", values="OrderCount").fillna(0)
    
    plt.figure(figsize=(14, 8))
    sns.heatmap(pivot_table, cmap="viridis", linewidths=.5)
    plt.title("Orders Heatmap by Country and Month", fontsize=20, fontweight='bold', pad=20)
    plt.tight_layout()
    plt.savefig(f"{FIGURES_DIR}/3d_orders.png")
    plt.close()
